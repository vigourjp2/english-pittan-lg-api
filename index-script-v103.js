...
const APP_VERSION='v113-board-full-winner';
...
let onlineEstablished=false;
let onlineJoinTimer=null;
let onlinePeerIds=new Set();
let desiredSeatIndex=null;
let serverSeatIndex=-1;
let serverRoomId='english';
let serverAuthoritative=false;
try{
  clientId = localStorage.englishClientId || '';
  if(!clientId){
    clientId = (__crypto && typeof __crypto.randomUUID==='function') ? __crypto.randomUUID().slice(0,8) : ('c'+Math.floor(Math.random()*999999));
    localStorage.englishClientId = clientId;
  }
}catch(e){
  clientId = (__crypto && typeof __crypto.randomUUID==='function') ? __crypto.randomUUID().slice(0,8) : ('c'+Math.floor(Math.random()*999999));
}
...
function defaultWsUrl(){
  ...
  let stored=''; try{ stored=localStorage.getItem('englishPittan.wsUrl')||''; }catch(e){}
  const explicit=(qs.get('ws')||stored||'').trim();
  if(explicit) return explicit;
  return 'wss://english-pittan-room.vigourjp2.workers.dev/room/english';
}
...
function fetchWithTimeout(url, options={}, timeoutMs=0){
  // v104: API判定のブラウザ側タイムアウトを撤廃。
  // Render cold start / Link Grammar / 外部判定が遅いだけで Abort され、
  // batch失敗→単体fallback→誤NG/ロールバックに落ちるのを防ぐ。
...
async function batchApiCheckItems(base, items, reasonPriorityEpoch){
  ...
  const res=await fetchWithTimeout(base+'/check-and-translate-batch',{
    ...
    body:JSON.stringify({
      ...
      // v104: サーバ側へ240件上限を要求しない。候補漏れによる一括判定の取りこぼしを防ぐ。
      translate:false,
      withTranslate:false,
      reasonMode:'none',
      reasonDisabled:true,
...
function freshState(playerCount=2){
  return {version:1,roomHostId:null,roomCreatedAt:null,gameId:'eng-'+Date.now().toString(36),turn:0,turnNo:1,playerCount,wordsMade:0,board:Array.from({length:N*N},()=>null),players:Array.from({length:playerCount},(_,i)=>({name:'Player '+(i+1),clientId:null,score:0,tiles:0,color:COLORS[i],hand:drawHand(i)})),log:[{t:Date.now(),text:'新規ゲーム開始',good:true}],statusText:'単語タイルを選んで空きマスに置く',statusClass:'info',lastJudgeMsg:'',lastJudgeClass:'info',gameOver:false,winnerIndex:null,winnerIndexes:[],endedAt:null,endReason:''};
}
...
function showToast(text,cls='info'){
  ...
  if(!el){
    ...
    el.id='toastMsg';
    el.style.cssText='position:fixed;left:16px;right:16px;bottom:18px;z-index:99999;padding:14px 16px;border-radius:16px;font-weight:900;font-size:16px;line-height:1.35;color:#fff;background:rgba(15,23,42,.96);border:2px solid rgba(148,163,184,.55);box-shadow:0 18px 50px rgba(0,0,0,.45);text-align:center;pointer-events:none;transform:translateY(20px);opacity:0;transition:.18s ease;';
    document.body.appendChild(el);
  }
  const border=cls==='bad'?'rgba(248,113,113,.95)':cls==='good'?'rgba(74,222,128,.95)':'rgba(56,189,248,.95)';
  el.style.borderColor=border;
  el.textContent=val;
  requestAnimationFrame(()=>{el.style.opacity='1';el.style.transform='translateY(0)';});
  clearTimeout(el._timer);
  el._timer=setTimeout(()=>{el.style.opacity='0';el.style.transform='translateY(20px)';},2600);
}
...
function playerDisplay(i){
  ...
  return nm && nm!==playerTag(i) ? `${playerTag(i)} ${nm}` : playerTag(i);
}
function normalizeSeatText(v){
  return String(v||'')
    .replace(/[Ｐｐ]/g,'P')
...
function preferredPlayerIndexFromName(){
  const raw=normalizeSeatText(($('nameInput')&&$('nameInput').value)||safeName()||'');
  const compact=raw.replace(/\s+/g,'');
  const m=/^P([1-4])$/.exec(compact) || /(?:^|[^A-Z0-9])P([1-4])(?:[^0-9]|$)/.exec(compact);
  return m ? Math.max(0, Math.min(3, Number(m[1])-1)) : -1;
}
function desiredPlayerIndex(){
  const live=preferredPlayerIndexFromName();
  if(live>=0){ desiredSeatIndex=live; return live; }
  if(Number.isInteger(desiredSeatIndex) && desiredSeatIndex>=0) return desiredSeatIndex;
  return -1;
}
function onlinePlayerCount(){
  const sel=Number($('playerCount').value)||2;
  return Math.max(2, sel);
}
function ensureOnlinePlayerCount(){
  const pc=Number($('playerCount').value)||1;
  if(pc<2){
    $('playerCount').value='2';
    setMsg('ONLINE対戦なので人数を2人対戦に切り替えました。P1端末とP2端末の両方でONLINEにしてください。','info');
  }
}
function canHostControl(){
  if(!isOnlineGame()) return true;
  return serverAuthoritative ? serverSeatIndex===0 : localPlayerIndex()===0;
}
function clearLocalClientSeats(){
  if(!state||!Array.isArray(state.players)) return;
  state.players.forEach(p=>{ if(String(p?.clientId||'')===String(clientId)) p.clientId=null; });
}
function startOnlineRoomAsHost(){
  ensureOnlinePlayerCount();
  const pc=onlinePlayerCount();
  state=freshState(pc);
  state.roomHostId=clientId;
  state.roomCreatedAt=Date.now();
  state.players[0].clientId=clientId;
  state.players[0].name=safeName();
  localStorage.englishName=safeName();
  selectedHandIndex=-1;recentCells=[];failFocusCell=-1;state.attackChance=null;
  roomStateReceived=true;onlineEstablished=true;
  broadcast({type:'englishNewGame',gameId:state.gameId,state:lightState(),roomHostId:clientId,reason:'host-start'});
  setMsg('この端末がP1です。P2以降はONLINE接続だけで参加します。','good');
  render();
}
function maybeStartOnlineRoomAfterDiscovery(){
  if(roomStateReceived||onlineEstablished) return;
  ensureOnlinePlayerCount();
  const ids=Array.from(onlinePeerIds||[]).filter(Boolean).sort();
  const leader=ids[0]||clientId;
  if(String(leader)===String(clientId)){
    startOnlineRoomAsHost();
  }else{
    clearLocalClientSeats();
    requestSeat('leader-wait');
    setMsg('部屋ホストの状態を待っています。席はホストから割り当てられます。','info');
    render();
  }
}
function playerIndexForClient(id=clientId){
  const cid=String(id||'');
  if(!cid || !state || !Array.isArray(state.players)) return -1;
  return state.players.findIndex(p=>String(p?.clientId||'')===cid);
}
function localPlayerIndex(){
  if(isOnlineGame() && serverAuthoritative && serverSeatIndex>=0) return serverSeatIndex;
  const found=playerIndexForClient(clientId);
  if(found>=0) return found;
  if(state?.playerCount===1) return 0;
  return -1;
}
function localPlayer(){
  const i=localPlayerIndex();
  return i>=0 ? state.players[i] : null;
}
function claimLocalPlayerSeat(preferIndex=0, announce=true){
  if(!state || !Array.isArray(state.players)) return -1;
  // v109: 名前欄の P1/P2 で席を決めない。席はオンライン部屋の空き順で決める。
  const existing=playerIndexForClient(clientId);
  if(existing>=0){
    state.players[existing].name=safeName();
    return existing;
  }
  let target=state.players.findIndex(p=>!p.clientId);
  if(target<0) return -1;
  state.players[target].clientId=clientId;
  state.players[target].name=safeName();
  if(announce) broadcast({type:'englishState',gameId:state.gameId,state:lightState(),reason:'seat-claim'});
  return target;
}
function assignRemoteSeat(remoteId, remoteName=''){
  if(!state || !Array.isArray(state.players)) return -1;
  const rid=String(remoteId||'');
  if(!rid || rid===String(clientId)) return -1;
  const existing=state.players.findIndex(p=>String(p?.clientId||'')===rid);
  if(existing>=0){
    if(remoteName) state.players[existing].name=String(remoteName).trim().slice(0,18);
    return existing;
  }
  const target=state.players.findIndex(p=>!p.clientId);
  if(target<0) return -1;
  state.players[target].clientId=rid;
  state.players[target].name=String(remoteName||('Player '+(target+1))).trim().slice(0,18);
  return target;
}
function isOnlineGame(){ return !!(socket && socket.readyState===WebSocket.OPEN); }
function isMyTurn(){
  if(!state || state.playerCount<=1) return true;
  // 未接続時は1端末で交代プレイできるようにする。オンライン接続中だけ担当プレイヤーを厳密判定。
  if(!isOnlineGame()) return true;
  if(serverAuthoritative && serverSeatIndex<0) return false;
  return localPlayerIndex() === (state.turn%state.playerCount);
}
function isBoardEmpty(){ return !state.board.some(Boolean); }
function cornerIndexes(){ return [0,N-1,N*(N-1),N*N-1]; }
function isAttackCorner(i){ return cornerIndexes().includes(i) && i!==0; }
function isAttackChanceActiveFor(pi=state.turn%state.playerCount){
  return !!(state.attackChance && state.attackChance.playerIndex===pi);
}
function hasAdjacentCard(cellIndex){
  const p=xy(cellIndex);
  for(let dy=-1;dy<=1;dy++){
    for(let dx=-1;dx<=1;dx++){
      if(dx===0 && dy===0) continue;
      const x=p.x+dx,y=p.y+dy;
      if(x>=0&&y>=0&&x<N&&y<N&&state.board[idx(x,y)]) return true;
    }
  }
  return false;
}
function canPlaceAt(cellIndex){
  if(state?.gameOver) return {ok:false,msg:'ゲーム終了済みです。新規ゲームで再開してください'};
  const pi=state.turn%state.playerCount;
  const attack=isAttackChanceActiveFor(pi);
  const occupied=!!state.board[cellIndex];
  if(occupied && !attack) return {ok:false,msg:'そこは埋まってる'};
  if(isBoardEmpty()) return cellIndex===0 ? {ok:true} : {ok:false,msg:'最初の1枚は左上の隅だけです'};
  if(!occupied && !hasAdjacentCard(cellIndex)) return {ok:false,msg:'既存カードに隣接した場所だけ置けます'};
  if(occupied && attack) return {ok:true,overwrite:true};
  return {ok:true};
}
function advanceAfterPlacement(cellIndex, wasAttackChance){
  const pi=state.turn%state.playerCount;
  const earnsAttack=!wasAttackChance && isAttackCorner(cellIndex);
  if(wasAttackChance){
    state.attackChance=null;
    nextTurn();
  }else if(earnsAttack){
    state.attackChance={playerIndex:pi,earnedAt:Date.now(),corner:cellIndex};
    state.turnNo++;
    addLog(`${state.players[pi]?.name||'Player'}: アタックチャンス獲得！もう1回置けます`,true);
    setMsg('アタックチャンス！続けてもう1枚置けます。既存カードへの上書きも可能です。','good');
  }else{
    state.attackChance=null;
    nextTurn();
  }
...
async function showSentenceGeneratedImage(matches, index=0) {
  ...
  for(const m of (matches || [])){
    ...
    if(m.gameOk !== true && m.ok !== true) continue;
...
async function placeTile(cellIndex){
  ...
  if(state?.gameOver){
    setMsg('ゲーム終了済みです。新規ゲームで再開してください。','bad');
    selectedHandIndex=-1;
    render();
    return;
  }
  if(!isMyTurn()){
    const tp=currentPlayer();
    setMsg(`今は${tp?.name||'他プレイヤー'}のターンです。自分のターンまで待ってください。`,'bad');
    selectedHandIndex=-1;
    render();
    return;
  }
  const placementRule=canPlaceAt(cellIndex);
  if(!placementRule.ok){setMsg(placementRule.msg,'bad');return;}
  if(selectedHandIndex<0){setMsg('先に手札を選んで！','bad');return;}
  const p=currentPlayer();
  const actorIndex=state.turn%state.playerCount;
  const wasAttackChance=isAttackChanceActiveFor(actorIndex);
  ...
  // v104: API一括判定の30秒watchdogを撤廃。
  // ここで仮配置を巻き戻すと、遅れて返った正常なbatch結果を捨ててしまい、
  // 成立文がタイムアウトNGになる。配置ロックはAPI応答完了まで維持する。
  ...
  clearResultOverlays();
  state.board[cellIndex]={w:word,owner:actorIndex,baseOwner:actorIndex,at:Date.now(),useCount:0,unusedPenaltyApplied:false,overwritten:!!placementRule.overwrite};
  ...
  setMsg('API一括判定中…。候補を全件確認しています。','info');
  render();
  let made=[];
  try{
  ...
  recalcTiles();
  if(isBoardFull()){
    finishGame('board-full');
  }else{
    advanceAfterPlacement(cellIndex, wasAttackChance);
  }
  selectedHandIndex=-1;
...
function toggleSelectedThirdPerson(){
  if(state?.gameOver){setMsg('ゲーム終了済みです。新規ゲームで再開してください','bad');return;}
  if(placementJudgeBusy){setMsg('判定中…。結果が出てから手札を変化してください。','info');return;}
  if(!isMyTurn()){setMsg('自分のターンではありません','bad');return;}
  const p=currentPlayer();
  if(!p){return;}
...
function isBoardFull(){return !!(state?.board?.length) && state.board.every(Boolean);}
function sortedPlayersForWin(){
  return (state.players||[]).map((p,i)=>({i,name:p?.name||`Player ${i+1}`,color:p?.color||COLORS[i]||'#fff',tiles:Number(p?.tiles||0),score:Number(p?.score||0)}))
    .sort((a,b)=>(b.tiles-a.tiles)||(b.score-a.score)||(a.i-b.i));
}
function finishGame(reason='board-full'){
  recalcTiles();
  const ranking=sortedPlayersForWin();
  const top=ranking[0];
  if(!top) return false;
  const winners=ranking.filter(x=>x.tiles===top.tiles && x.score===top.score);
  state.gameOver=true;
  state.winnerIndex=top.i;
  state.winnerIndexes=winners.map(x=>x.i);
  state.endedAt=Date.now();
  state.endReason=reason;
  state.attackChance=null;
  selectedHandIndex=-1;
  const label=winners.length>1 ? `DRAW: ${winners.map(x=>playerDisplay(x.i)).join(' / ')}` : `WINNER: ${playerDisplay(top.i)}`;
  addLog(`${label} / 陣地${top.tiles} / ${top.score}点`,true);
  setMsg(`${label}！ 陣地${top.tiles} / ${top.score}点`, 'good');
  playWinnerFx(ranking);
  return true;
}
function ensureWinnerOverlay(){
  let el=document.getElementById('winnerOverlay');
  if(el) return el;
  el=document.createElement('div');
  el.id='winnerOverlay';
  el.style.cssText='position:fixed;inset:0;z-index:90;display:none;align-items:center;justify-content:center;padding:18px;background:radial-gradient(circle at 50% 20%,rgba(250,204,21,.20),rgba(3,7,18,.76) 45%,rgba(3,7,18,.92));backdrop-filter:blur(6px);pointer-events:none;';
  el.innerHTML='<div id="winnerCard" style="width:min(620px,94vw);border-radius:28px;padding:22px;text-align:center;background:linear-gradient(135deg,rgba(15,23,42,.96),rgba(30,41,59,.92));border:2px solid rgba(250,204,21,.72);box-shadow:0 30px 100px rgba(0,0,0,.65),0 0 55px rgba(250,204,21,.35);"><div id="winnerTitle" style="font-size:38px;font-weight:1000;letter-spacing:.04em;color:#fde68a;text-shadow:0 3px 16px rgba(0,0,0,.5);">WINNER!</div><div id="winnerName" style="font-size:28px;font-weight:1000;margin-top:8px;"></div><div id="winnerScore" style="font-size:18px;font-weight:900;color:#bbf7d0;margin-top:8px;"></div><div id="winnerRank" style="margin-top:14px;display:grid;gap:6px;"></div><div style="margin-top:16px;font-size:13px;color:#dbeafe;">盤面がすべて埋まったためゲーム終了。陣地数優先、同数なら得点で判定。</div></div>';
  document.body.appendChild(el);
  return el;
}
function updateWinnerOverlay(){
  const el=ensureWinnerOverlay();
  if(!state?.gameOver){ el.style.display='none'; return; }
  const ranking=sortedPlayersForWin();
  const top=ranking[0];
  const winners=Array.isArray(state.winnerIndexes)&&state.winnerIndexes.length ? state.winnerIndexes : [state.winnerIndex];
  ...
  const rank=document.getElementById('winnerRank');
  if(title) title.textContent=draw?'DRAW!':'WINNER!';
  if(name){ name.textContent=draw?winners.map(i=>playerDisplay(i)).join(' / '):playerDisplay(top?.i??state.winnerIndex??0); name.style.color=top?.color||'#fde68a'; }
  if(score) score.textContent=top?`陣地 ${top.tiles} / 得点 ${top.score}`:'';
  if(rank) rank.innerHTML=ranking.map((r,idx)=>`<div style="display:grid;grid-template-columns:42px 1fr auto;gap:8px;align-items:center;padding:8px 10px;border-radius:13px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);"><b style="color:#fde68a">#${idx+1}</b><span style="text-align:left;color:${r.color};font-weight:1000">${esc(playerDisplay(r.i))}</span><span style="font-weight:1000">陣地${r.tiles} / ${r.score}点</span></div>`).join('');
}
function playWinnerFx(ranking){
  try{playChime(true);setTimeout(()=>playChime(true),260);}catch(e){}
  const top=ranking?.[0];
  const fx=$('fxBanner');
  if(fx){
    fx.classList.add('combo');
    fx.querySelector('.fxBig').textContent='WINNER!';
    $('fxEn').textContent=top?playerDisplay(top.i):'GAME SET';
    $('fxJa').textContent=top?`陣地 ${top.tiles} / 得点 ${top.score}`:'盤面がすべて埋まりました';
    $('fxPts').textContent='GAME SET';
    fx.classList.remove('show');void fx.offsetWidth;fx.classList.add('show');
  }
  updateWinnerOverlay();
  const cx=innerWidth/2, cy=innerHeight*.36;
  for(let i=0;i<72;i++){const sp=document.createElement('i');sp.className='spark';sp.style.left=cx+'px';sp.style.top=cy+'px';const a=Math.random()*Math.PI*2,d=70+Math.random()*220;sp.style.setProperty('--dx',Math.cos(a)*d+'px');sp.style.setProperty('--dy',Math.sin(a)*d+'px');document.body.appendChild(sp);setTimeout(()=>sp.remove(),1100);}
}
function lightState(){return JSON.parse(JSON.stringify(state));}
function roomHostIdOf(s){ return String(s?.roomHostId || ''); }
function roomCreatedAtOf(s){ return Number(s?.roomCreatedAt || 0); }
function compareRoomAuthority(a,b){
  const ah=roomHostIdOf(a), bh=roomHostIdOf(b);
  if(ah && !bh) return -1;
  if(!ah && bh) return 1;
  if(!ah && !bh) return 0;
  const at=roomCreatedAtOf(a), bt=roomCreatedAtOf(b);
  if(at && bt && Math.abs(at-bt)>50) return at<bt ? -1 : 1;
  if(ah!==bh) return ah<bh ? -1 : 1;
  return 0;
}
function isLocalRoomHost(){ return isOnlineGame() && String(state?.roomHostId||'')===String(clientId); }
function requestSeat(reason='seat-request'){
  if(!isOnlineGame()) return false;
  return broadcast({type:'englishSeatRequest',wantState:true,gameId:state?.gameId||'',roomHostId:state?.roomHostId||'',reason});
}
function shouldAcceptIncomingRoom(s){
  if(!s||!Array.isArray(s.players)) return false;
  const incomingHost=roomHostIdOf(s), currentHost=roomHostIdOf(state);
  const incomingHasMe=Array.isArray(s.players)&&s.players.some(p=>String(p?.clientId||'')===String(clientId));
  const currentHasMe=playerIndexForClient(clientId)>=0;
  if(!onlineEstablished || !currentHost) return true;
  if(incomingHost && currentHost && incomingHost!==currentHost){
    return compareRoomAuthority(s,state)<0;
  }
  if(incomingHost && currentHost && incomingHost===currentHost){
    if(s.gameId!==state.gameId) return true;
    const incomingTurn=Number(s.turnNo)||0;
    const currentTurn=Number(state.turnNo)||0;
    return incomingHasMe || !currentHasMe || incomingTurn>=currentTurn;
  }
...
function applyState(s,why='同期'){
  ...
  const incomingHadSeat=Array.isArray(s.players) && s.players.some(p=>String(p?.clientId||'')===String(clientId));
  state=s;selectedHandIndex=-1;recentCells=[];failFocusCell=-1;
  if(!state.roomHostId){
    const host=(state.players||[]).find(p=>p&&p.clientId);
    state.roomHostId=host?.clientId||null;
  }
  if(!state.roomCreatedAt) state.roomCreatedAt=Date.now();
  const hasSeat=playerIndexForClient(clientId)>=0;
  recalcTiles();
  if(!hasSeat && isOnlineGame() && !serverAuthoritative) requestSeat('apply-state-no-seat');
  setMsg(`${why} / 自分は${hasSeat?playerTag(playerIndexForClient(clientId)):'席割当待ち'}です`,'good');render();
  return hasSeat && !incomingHadSeat;
}
function passTurn(){if(state?.gameOver){setMsg('ゲーム終了済みです。新規ゲームで再開してください','bad');return;}if(!isMyTurn()){setMsg('自分のターンではありません','bad');return;}const p=currentPlayer();addLog(`${p.name}: パス`,false);state.attackChance=null;nextTurn();broadcast({type:'englishState',gameId:state.gameId,state:lightState(),reason:'pass'});render();}
function swapHand(){if(state?.gameOver){setMsg('ゲーム終了済みです。新規ゲームで再開してください','bad');return;}if(!isMyTurn()){setMsg('自分のターンではありません','bad');return;}const p=currentPlayer();p.hand=drawHand(99);p.handMeta=[];p.score=Math.max(0,p.score-5);addLog(`${p.name}: 手札入替 -5`,false);state.attackChance=null;nextTurn();broadcast({type:'englishState',gameId:state.gameId,state:lightState(),reason:'swap'});render();}
function newGame(broadcastIt=true){
  if(broadcastIt && isOnlineGame() && !canHostControl()){
    setMsg('新規ゲーム開始はP1端末だけです。P2以降はP1の開始を待ってください。','bad');
    return;
  }
  clearEnglishJudgeCache();displayedRejectReasonRequestKeys=new Set();displayedRejectReasonPlacementKeys=new Set();if(isOnlineGame())ensureOnlinePlayerCount();const pc=isOnlineGame()?onlinePlayerCount():(Number($('playerCount').value)||2);const prevRoomCreatedAt=state?.roomCreatedAt||Date.now();state=freshState(pc);state.roomHostId=isOnlineGame()?clientId:null;state.roomCreatedAt=isOnlineGame()?prevRoomCreatedAt:null;const name=safeName();const mySeat=(isOnlineGame()&&serverAuthoritative&&serverSeatIndex>=0)?serverSeatIndex:0;state.players[mySeat].name=name;state.players[mySeat].clientId=clientId;localStorage.englishName=name;selectedHandIndex=-1;recentCells=[];failFocusCell=-1;state.attackChance=null; if(broadcastIt)broadcast({type:'englishNewGame',gameId:state.gameId,state:lightState()});render();setMsg(state.playerCount===1?'1人練習開始。最初の1枚は左上です。':'新規ゲーム開始。この端末が部屋ホストP1です。以降の参加者はホストから席を受け取ります。','good');}
function render(){
  ...
  const lp=online ? localPlayer() : tp;
  const lpIndex=online ? localPlayerIndex() : (state.turn%state.playerCount);
  const myTurn=isMyTurn() && !state.gameOver;
  const attackNow=!state.gameOver && isAttackChanceActiveFor(state.turn%state.playerCount);
  $('turnPlayer').textContent=tp?`${playerDisplay(state.turn%state.playerCount)}${attackNow?'（アタックチャンス）':''}`:'-';$('turnPlayer').style.color=tp?tp.color:'#fff';
  const b=$('board');b.innerHTML='';state.board.forEach((c,i)=>{const d=document.createElement('button');const rule=canPlaceAt(i);d.className='cell '+(!c?'empty ':'')+(recentCells.includes(i)?'recent hit ':'');d.type='button';const routeNo=routeFocusCells.indexOf(i);if(routeNo>=0)d.classList.add('routeHit');if(i===failFocusCell)d.classList.add('failFocus');if(rule.ok&&selectedHandIndex>=0&&!placementJudgeBusy&&myTurn)d.classList.add('selectable');if(c){const ownerColor=state.players[c.owner]?.color||'#fff';if(c.pendingSentence)d.classList.add('unused');d.innerHTML=`<span class="owner" style="background:${ownerColor}"></span>${routeNo>=0?`<span class="routeStep">${routeNo+1}</span>`:''}<span class="word">${esc(c.w)}</span>${c.pendingSentence?'<span class="unusedMark">採点前</span>':''}<span class="pts">P${(c.owner||0)+1}</span>`;d.style.background=`linear-gradient(180deg, ${hexAlpha(ownerColor,.42)}, ${hexAlpha(ownerColor,.18)}, rgba(255,255,255,.05))`;d.style.borderColor=hexAlpha(ownerColor,.86);d.style.color='#fff';} d.onclick=()=>placeTile(i);b.appendChild(d);});
  const h=$('hand');h.innerHTML='';(lp?.hand||[]).forEach((w,i)=>{const pos=handInfo(w);const t=document.createElement('button');t.type='button';t.className='tile cat-'+pos.key+' '+(i===selectedHandIndex?'on':'')+(!myTurn?' used':'');t.innerHTML=`<span>${esc(w)}</span><small>${esc(displayHandLabel(w))}</small>`;t.onclick=()=>{if(state?.gameOver){setMsg('ゲーム終了済みです。新規ゲームで再開してください。','bad');return;}if(placementJudgeBusy){setMsg('判定中…。成立確定ではありません。結果が出るまで操作ロック中。','info');return;}if(!myTurn){setMsg(`ちょっと待って。今は${playerDisplay(state.turn%state.playerCount)}のターンです。あなたは${lpIndex>=0?playerTag(lpIndex):'観戦'}です。`,'bad');return;}selectedHandIndex=i;render();refreshFailReasonWithSelectedHand();};h.appendChild(t);});
  $('selectedInfo').textContent=lpIndex<0?'観戦中（空きプレイヤー枠なし）':(selectedHandIndex>=0?`${playerTag(lpIndex)} 選択: ${(lp.hand||[])[selectedHandIndex]}${(lp.handMeta&&lp.handMeta[selectedHandIndex])?'（3単現）':''}`:`${playerTag(lpIndex)} ${myTurn?'手札を選択':'待機中'}`);
  const ps=$('players');ps.innerHTML='';state.players.forEach((p,i)=>{const row=document.createElement('div');row.className='player '+(i===state.turn?'current':'')+(p.clientId===clientId?' me':'');row.style.borderColor=hexAlpha(p.color,.70);row.innerHTML=`<span class="dot" style="background:${p.color}"></span><span class="pname"><b style="color:${p.color}">${playerTag(i)}</b> ${esc(p.name||'')} <small>${p.clientId===clientId?'自分 / ':''}陣地${p.tiles||0}${state.attackChance?.playerIndex===i?' / AC':''}</small></span><span class="pscore">${p.score}</span>`;ps.appendChild(row);});
  ...
  $('targetList').innerHTML=`<span class="chip">StrictLG:${esc(linkGrammarStatus)}</span><span class="chip">判定:StrictLG+LT API</span><span class="chip">ターン本人のみ操作</span><span class="chip">初手左上・以降隣接</span><span class="chip">角でAC</span>`+targets.slice(0,10).map(t=>`<span class="chip">${esc(t)}</span>`).join('');
  updateWinnerOverlay();
  {const m=$('msg'); if(m){
...
function broadcast(obj){
  ...
  try{obj.clientId=clientId;obj.name=safeName();obj.seatIndex=serverSeatIndex;obj.playerCount=onlinePlayerCount();obj.appVersion=APP_VERSION;obj.time=Date.now();socket.send(JSON.stringify(obj));return true;}catch(e){return false;}
}
function connect(){
  if(socket&&(socket.readyState===WebSocket.OPEN||socket.readyState===WebSocket.CONNECTING))return;
  clearTimeout(retryTimer);clearInterval(heartbeat);setMp('CONNECTING','info');
  socket=new WebSocket(WS_URL);
  socket.addEventListener('open',()=>{ensureOnlinePlayerCount();setMp('ONLINE','good');roomStateReceived=false;onlineEstablished=false;serverAuthoritative=false;serverSeatIndex=-1;onlinePeerIds=new Set([clientId]);setMsg('ONLINE接続。サーバーから席番号を取得中…','info');broadcast({type:'englishJoin',wantState:true,gameId:state.gameId});clearTimeout(onlineJoinTimer);heartbeat=setInterval(()=>broadcast({type:'ping'}),10000);});
  socket.addEventListener('message',ev=>{rx++;$('rx').textContent=rx;handleNet(ev.data);});
  socket.addEventListener('close',()=>{clearInterval(heartbeat);serverAuthoritative=false;serverSeatIndex=-1;setMp('RETRY','bad');retryTimer=setTimeout(connect,3000);});
...
function handleNet(raw){eachPayload(raw,(msg)=>{
  if(!msg) return;
  if(msg.type==='seatAssigned'){
    serverAuthoritative=true;
    serverSeatIndex=Number.isFinite(Number(msg.seatIndex))?Number(msg.seatIndex):-1;
    serverRoomId=String(msg.roomId||'english');
    onlineEstablished=true;roomStateReceived=!!msg.state;
    if(onlineJoinTimer){clearTimeout(onlineJoinTimer);onlineJoinTimer=null;}
    if(Array.isArray(msg.seats)){ onlinePeerIds=new Set(msg.seats.filter(Boolean).map(s=>String(s.clientId)).filter(Boolean)); }
    if(msg.state){ applyState(msg.state,'サーバー同期'); }
    else if(msg.needNewGame && serverSeatIndex===0){ startOnlineRoomAsHost(); }
    else { setMsg(`ONLINE接続完了。自分は${serverSeatIndex>=0?playerTag(serverSeatIndex):'観戦'}です。P1の開始を待っています。`,'good'); render(); }
    return;
  }
  if(msg.type==='actionRejected'){
    setMsg(msg.reason||'サーバーで操作が拒否されました。','bad');
    if(msg.state) applyState(msg.state,'サーバー再同期');
    return;
  }
  if(msg.type==='roomPresence'){
    if(Array.isArray(msg.seats)) onlinePeerIds=new Set(msg.seats.filter(Boolean).map(s=>String(s.clientId)).filter(Boolean));
    return;
  }
  if(msg.type==='pong') return;
  if(!msg.server && msg.clientId===clientId)return;
  if(msg.clientId) onlinePeerIds.add(String(msg.clientId));
  if((msg.type==='englishHello'||msg.type==='englishSeatRequest')&&msg.wantState){
    if(serverAuthoritative){ broadcast({type:'englishJoin',wantState:true,gameId:state?.gameId||''}); return; }
    if(isLocalRoomHost()){
      const before=JSON.stringify((state.players||[]).map(p=>p.clientId||''));
      assignRemoteSeat(msg.clientId,msg.name);
      const after=JSON.stringify((state.players||[]).map(p=>p.clientId||''));
      broadcast({type:'englishState',gameId:state.gameId,state:lightState(),reason:before===after?'host-sync':'host-seat-assign'});
    }else if(onlineEstablished && state && state.roomHostId){
      requestSeat('non-host-forward-request');
    }
    return;
  }
  if(msg.type==='englishNewGame'&&msg.state){
    if(serverAuthoritative || shouldAcceptIncomingRoom(msg.state)) applyState(msg.state,'部屋に参加');
    else if(isLocalRoomHost()) broadcast({type:'englishState',gameId:state.gameId,state:lightState(),reason:'reject-foreign-newgame-resync'});
    return;
  }
  if((msg.type==='englishState'||msg.type==='englishPlace')&&msg.state){
    if(serverAuthoritative || shouldAcceptIncomingRoom(msg.state)){
      applyState(msg.state,'オンライン同期');
    }else if(isLocalRoomHost()){
      broadcast({type:'englishState',gameId:state.gameId,state:lightState(),reason:'authoritative-resync'});
    }
  }
});}
...
$('connectBtn').onclick=()=>{try{unlockSpeech();}catch(e){console.warn('unlockSpeech unavailable',e);}ensureOnlinePlayerCount();connect();};
$('newGameBtn').onclick=()=>newGame(true);
$('resetBtn').onclick=()=>{if(isOnlineGame()&&!canHostControl()){setMsg('盤面リセットはP1端末だけです。P2以降は待機してください。','bad');return;}if(confirm('盤面をリセットしてオンラインにも送信する？'))newGame(true);};
...
$('nameInput').addEventListener('change',()=>{localStorage.englishName=safeName();const pi=localPlayerIndex();if(pi>=0)state.players[pi].name=safeName();broadcast({type:'englishState',gameId:state.gameId,state:lightState(),reason:'rename'});render();});
$('playerCount').addEventListener('change',()=>{if(isOnlineGame()){setMsg('人数変更は接続前、またはP1が新規ゲーム開始時に行ってください。','bad');return;}newGame(false);});
